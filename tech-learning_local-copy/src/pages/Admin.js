...
function MyVerticallyCenteredModal(props) {
  ...
  return (
    <Modal
      ...
      className="rounded">
      <Modal.Body>
        <h4 className="text-center text-white p-0 m-0 fw-bold">
          User Enrroled Courses
        </h4>
      ...
      <div className="p-2">
        <Button className="app-nav__btn" onClick={props.onHide}>
          Close
        </Button>
      </div>
    </Modal>
...
function Admin() {
  ...
  const postCourse = (e) => {
    e.preventDefault();
    axios
      .post("http://localhost:7000/maincards", body)
      .then(() => {
            ...
            <tr key={user.id}>
              ...
              <td>
                {user.position === "admin" ? (
                  ...
                  <>
                    <Button
                      className="mt-3 m-auto app-nav__btn"
                      onClick={() => {
                        setModalShow(true);
        ...
        <Form>
          ...
          <Form.Group className="mb-3" controlId="formBasicSrc">
            ...
            <Form.Control
              type="file"
              value=""
              onChange={(e) => ImgfileReader(e)}
              required
              placeholder="Enter Course img src"
            />
          </Form.Group>
          <Button
            variant="primary"
